...
export const IS_MOBILE_DEVICE = typeof navigator !== 'undefined' &&
...
export async function agentFetch(path, opts = {}) {
  if (!AGENT_CONNECTED) {
...
export function getAudioUrl(file, useAgent) {
  if (useAgent) {
...
export function formatSmallMeta(f) {
  if (!f) return ''
  const parts = []
...
export function normDupeKey(filename) {
...
function App() {
  ...
  const playLibraryPreview = (file) => {
    ...
    const candidates = []
    if (query) {
      // IGUAL que Discovery: SoundCloud (tema COMPLETO, version de DJ correcta y
      // larga ~90s+) primero. iTunes (preview_url, 30s, a veces otra version) queda
      // de fallback para lo que SoundCloud no encuentra.
...
export function SwipeableRow({ children, onReveal }) {
